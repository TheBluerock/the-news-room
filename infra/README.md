...
Validate (use the `admin_user` from `group_vars/all.yml`, default `deploy`):
```sh
ssh <admin_user>@<vps-ip> 'docker node ls'
```
...
### 3b. Vault AppRole secrets (after first Vault unseal)

Vault AppRole `role_id` / `secret_id` per service do **not** exist yet at this point. They are
generated by Vault itself after the cluster is deployed, initialised, and unsealed, and after
the AppRole auth method is configured per service.

Bootstrap order:

1. Deploy the stack (step 5 below) — Vault starts sealed.
2. Initialise + unseal Vault (`vault operator init`, `vault operator unseal`).
3. Configure AppRole auth and policies per service per `ops/VAULT-DR.md`.
4. For each service, fetch the generated values:
   ```sh
   vault read -field=role_id   auth/approle/role/<svc>/role-id
   vault write -f -field=secret_id auth/approle/role/<svc>/secret-id
   ```
5. Create the Docker secrets with those retrieved values:
   ```sh
   for svc in auth learner agent moderation analytics sanity; do
     printf '%s' "$ROLE_ID_${svc}"   | docker secret create "vault_role_id_${svc}"   -
     printf '%s' "$SECRET_ID_${svc}" | docker secret create "vault_secret_id_${svc}" -
   done
   printf '%s' "$VAULT_ROOT_OR_BACKUP_TOKEN" | docker secret create vault_token -
   ```
6. Redeploy the stack so app services pick up the new secrets.

Full procedure: `ops/VAULT-DR.md`.
